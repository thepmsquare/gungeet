...
import "./stylesheets/CreateIPD.css";
import {
  Typography,
  TextField,
  Divider,
...
class CreateIPD extends Component {
  constructor(props) {
    super(props);
    this.state = {
      billDate: Date.parse(new Date().toLocaleDateString()),
    ...
    if (
      ...
      if (this.state.admissionDate > this.state.dischargeDate) {
        ...
        db.collection("IPD")
          ...
          .then(() => {
            document.querySelector(".CreateIPD-Hidden").click();
            this.setState(() => {
              return {
                billDate: Date.parse(new Date().toLocaleDateString()),
                admissionDate: Date.parse(new Date().toLocaleDateString()),
  ...
  render = () => {
    return (
      <div className="CreateIPD">
        <Typography variant={isBrowser ? "h1" : "h3"}>New IPD Bill</Typography>
        <form
          autoComplete="off"
          onSubmit={this.handleFormSubmit}
          className="CreateIPD-Form"
        >
          <TextField
            label="Bill Number"
            type="number"
          ...
          <Divider />
          <div className="CreateIPD-FormDiv1">
          ...
          <div className="CreateIPD-FormDiv2">
          ...
          <div className="CreateIPD-FormDiv3">
